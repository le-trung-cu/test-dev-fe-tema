<template>
  <div class="pt-[160px] md:pt-[100px]">
    <div class="map max-w-[1160px] mx-auto aspect-[2250/1500]">
      ...
      <img ref="animatedElement" :class="{ visible: isVisible, hiddenX: !isVisible }"
        class="absolute magician z-10 w-[300px] left-1/2 -translate-x-1/2 -translate-y-full top-[55%] md:top-1/2 lg:top-[33%]"
        src="/magician.png" />
    </div>

    <h2 class="text-center text-[2.5rem] md:text-[3.75rem] font-black font-playfair mt-10 md:mt-0">{{
      $t('our_games_title') }}</h2>
    <p class="text-muted-foreground font-normal text-sm text-center mt-5 max-w-[840px] mx-auto">
      {{ $t('our_games_paragraph') }}
    </p>
    <div class="flex flex-wrap px-2 py-14 container mx-auto">
      <div v-for="chunk in [0, 1, 2, 3]" :key="chunk"
        :class="{ 'basis-1/2 md:basis-1/4': true, 'order-1': chunk === 0, 'order-3 -mt-14 md:order-2 md:mt-14': chunk === 1, 'order-2 mt-14 md:order-3 md:mt-0': chunk === 2, 'order-4 md:mt-14': chunk === 3 }">
        <div v-for="item of listGames.slice(chunk * 3, chunk * 3 + 3)" :key="item.title" class="p-2">
